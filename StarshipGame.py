...
from arcade.sound import stop_sound
...
class StarShip(arcade.Sprite):
    ...
    def Fire(self):
        ...
        arcade.play_sound(arcade.sound.Sound(':resources:sounds/laser3.wav'), 0.2, 0,False)
...
class Game(arcade.Window):
    def __init__(self):
        ...
        self.background_img = arcade.load_texture("stars.jpg")
        self.me = StarShip()
        self.enemy_list = []
        self.my_thread_dead = False
        self.my_thread = Thread(target=self.Add_enemy)
        self.game_sound = arcade.sound.Sound("main_sound.mp3")
        self.game_over = False
        self.my_thread.start()

    def Add_enemy(self):
        while True:
            if self.my_thread_dead:
                print("Thread Dead")
                break
            else:
                arcade.play_sound(self.game_sound,1,0,False)
                self.enemy_list.append(Enemy())
                sleep(Random_Enemy_arrival_time())
        
    def on_draw(self):
        arcade.start_render()
        if self.me.health < 1:
            del(self.game_sound)
            self.my_thread_dead = True
        ...
        for enemy in self.enemy_list:
            for bullet in self.me.bullet_list:
                if arcade.check_for_collision(bullet, enemy):
                    arcade.play_sound(arcade.sound.Sound(':resources:sounds/hit1.wav'),0.5,0,False)
